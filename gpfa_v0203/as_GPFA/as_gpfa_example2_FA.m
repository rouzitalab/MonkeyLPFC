% =====
% as_gpfa_example2_FA
% example 2 does analysis on simulated array data generated from as_SimSaccadeExp
% tt: does statistical test on 2 groups instead of 
% decoder
% TIPS
% =====
...
dat1 = as_SimSaccadeExp(80,0.5);
% dat1 = as_shuffleTrajectories_dat1(dat1);
...
%% HACK to try to get shuffle ajs Dec 2011
[Y,I]=sort(index1(:,1));
%Y=index1(:,1);
dat=dat1;
if 1
...
% outIdx=as_plot2D(trialDirLookup, seqTrain, 'xorth', 'dimsToPlot', 1:2);
...
%%
for i = 1:size(dat,2)
    Trajs.Y{i}=[seqTrain1(i).xorth(1,:);seqTrain1(i).xorth(2,:)]';
end
if 1 % method(1:2) == 'gp'
    ops.K=8;
    ops.zero='none';
    ...
    [orderedGroupEst,map, cm] = as_classify3(origGroup,unorderedGroupEst,3)
    H = as_infoClust1(cm);
    Pc = sum(diag(cm)) / sum(sum(cm));
elseif 0 % method(1:2) == 'fa'
    P=zeros(size(dat,2),2);
    for i = 1:size(dat,2)
        P(i,:)=mean(Trajs.Y{i});
    end
    numComponents=8;
    options = statset('Display','off'); %'final'
    obj = gmdistribution.fit(P,numComponents,'Options',options,'SharedCov',1);% was 2
    idx = cluster(obj,P);
