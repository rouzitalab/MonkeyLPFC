...
% _attemptold is an attempt to reconstruct the older version that makes 
% neural trajectories for exp 1

runIdx = 959;
analysisBlock=[3]; %[1 8];%4 or 7 is all ; these are groups of analysis Periods
analysisGroup=[];
shuffleIdx=0;
%%
...
if 0
analysisGroup{1}.aPeriods=[ 66];%106 % always put lower number here (its a bug)
analysisGroup{1}.trialIntervals = [200 850]; 
analysisGroup{2}.aPeriods=[70];  %these are in analysis Periods
analysisGroup{2}.trialIntervals = [200 850];
end

excludeNoiseUnits = 1;
ops.K=8;  %# of allowable clusters
filename =  'sra3_2_j_039_00+01.resmat' %'sra3_2_j_039_00+01.resmat'%'
%filename =  'sra3_2_j_039_00+02.resmat'
...
% dat1=as_convertres2dat1(filename,analysisBlock,excludeNoiseUnits);
 dat1=as_convertres2dat2_attempt(filename,analysisBlock,excludeNoiseUnits,analysisGroup);
...
xDim = 2; %coincidence ajs normally 8
...
%as_plot2DTrajDistVersusTime(trialDirLookup, seqTrain, 'xorth', 'dimsToPlot', [1 2]);
...
%   end
 for i = 1:size(dat,2)
     Trajs.Y{i}=[seqTrain1(i).xorth(1,:);seqTrain1(i).xorth(2,:)]'; % TRY FITTING TO MULTIDIMENSION
%   Trajs.X{i}=[seqTrain1(i).xorth(1,:)]'; % TRY FITTING TO MULTIDIMENSION
%  Trajs.Y{i}=[seqTrain1(i).xorth(2,:)]'; 
 end
%%
ops.K=ops.K;
...
ops.method = 'lrm'; %'lrm' 'gmix' 'srm' ListModels()
ops.IterLimit=200;
ops.NumEMStarts=2;
ops.ShowGraphics	=0;
...
as_plotEachDimVsTime(trialDirLookup,seqTrain, 'xorth', result.binWidth);
% NOTES:
