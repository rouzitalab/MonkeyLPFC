...
% tmp = uint8(randperm(length(learned)));
% tmp2 = tmp(1:length(unlearned));
% flag = learned(tmp2);
kind=1;
% flag = learned;
flag = unlearned;
...
spikes = data1.spikes;
times = data1.times;
% if w==0, idx = find (times < 0);
% elseif w==1, idx = find (0 < times & times < 0.25);
% elseif w==2, idx = find (0.25 < times & times < 1.25);
% elseif w==3, idx = find (times > 1.25);
% end
...
for i = 1:trials*samples
    if direction(i)==0 && col(i)==1, rul(i)=1;
    elseif direction(i)==0 && col(i)==2, rul(i)=2;
    elseif direction(i)==0 && col(i)==3, rul(i)=3;
    elseif direction(i)==1 && col(i)==1, rul(i)=4;
    elseif direction(i)==1 && col(i)==2, rul(i)=5;
    elseif direction(i)==1 && col(i)==3, rul(i)=6;
    elseif direction(i)==2 && col(i)==1, rul(i)=7;
    elseif direction(i)==2 && col(i)==2, rul(i)=8;
    elseif direction(i)==2 && col(i)==3, rul(i)=9;
    elseif direction(i)==3 && col(i)==1, rul(i)=10;
    elseif direction(i)==3 && col(i)==2, rul(i)=11;
    elseif direction(i)==3 && col(i)==3, rul(i)=12;
    elseif direction(i)==4 && col(i)==1, rul(i)=13;
    elseif direction(i)==4 && col(i)==2, rul(i)=14;
    elseif direction(i)==4 && col(i)==3, rul(i)=15;
    elseif direction(i)==5 && col(i)==1, rul(i)=16;
    elseif direction(i)==5 && col(i)==2, rul(i)=17;
    elseif direction(i)==5 && col(i)==3, rul(i)=18;
    elseif direction(i)==6 && col(i)==1, rul(i)=19;
    elseif direction(i)==6 && col(i)==2, rul(i)=20;
    elseif direction(i)==6 && col(i)==3, rul(i)=21;
    elseif direction(i)==7 && col(i)==1, rul(i)=22;
    elseif direction(i)==7 && col(i)==2, rul(i)=23;
    else, rul(i)=24;
    end
end
tmp = unique(rul);
...
prs.varname = {'Saccade', 'Target', 'Rule'};
prs.vartype = {'1D', '1D', '1D'};
prs.basistype = {'raisedcosine','raisedcosine','raisedcosine'};
prs.nbins = {8,8,length(unique(rul))}; % number of different labels in our case
prs.binrange = [];
prs.nfolds = 10;
prs.dt = 0.01; % 1/sampling_rate
prs.filtwidth = 10;
prs.linkfunc = 'log';
prs.lambda = {10, 10, 10};
prs.alpha = 0.05;
prs.varchoose = [0 0 0]; % auto choice which variable is encoded by yt
