...
block = [data1.border(1), data1.border(2)];
learned = data1.learned + 1;%idx = learned > block(1) & learned < block(2);learned = learned(idx);
unlearned = data1.unlearned + 1;%idx = unlearned > block(1) & unlearned < block(2);unlearned = unlearned(idx);
% flag = learned;
flag = unlearned;
...
tmp = unique(sac);
tmp2 = 1:length(tmp);
tmp3 = zeros(length(sac),1);
for i = 1 : length(sac)
    for j = 1 : length(tmp)
        if sac(i)==tmp(j), tmp3(i)=tmp2(j); end
    end
end
sac = tmp3;
% trg = trg';
...
prs.varname = {'Saccade','Rule'};
prs.vartype = {'1D','1D'};
prs.basistype = {'raisedcosine','raisedcosine'};
prs.nbins = {2,2}; % number of different labels in our case
prs.binrange = [];
prs.nfolds = 10;
...
prs.lambda = {10, 10};
prs.alpha = 0.05;
prs.varchoose = [0 0]; % auto choice which variable is encoded by yt
