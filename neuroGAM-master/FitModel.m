...
%     fprintf('\t\t- Cross validation fold %d of %d\n', k, nfolds);
    ...
    smooth_spikes_test = convn(test_spikes,h,'same'); %returns vector same size as original - modified by Diego
    ...
    smooth_spikes_train = convn(train_spikes,h,'same'); %returns vector same size as original- modified by Diego
    ...
    smooth_fr_hat_test = convn(fr_hat_test,h,'same'); %returns vector same size as original-modified by diego
...
%     smooth_fr_hat_train = conv3(fr_hat_train,h,'same'); %returns vector same size as original
    smooth_fr_hat_train = convn(fr_hat_train,h,'same'); %returns vector same size as original-modified by diego
