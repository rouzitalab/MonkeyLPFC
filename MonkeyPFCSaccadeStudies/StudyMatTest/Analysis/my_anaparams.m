...
ana_win(end).vBar = 250;
ana_win(end).colour = 'b';

% Delay window.
ana_win(end+1).name = 'cue';
ana_win(end).timeLockEvent = 'targetOnset';
ana_win(end).winEdges = [251 1300];
ana_win(end).plotX = [251 1300];
ana_win(end).avoidEvent = 'fixationOffset';
ana_win(end).avoidWin = [-200 Inf];
...
analysisParams(end).critFlip = 'fixationOffset';  % I'm OK with saccades that come after cueOffset but before fixationOffset
