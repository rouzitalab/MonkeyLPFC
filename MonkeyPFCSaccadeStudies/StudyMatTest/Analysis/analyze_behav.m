...
%     end
    ...
    %% Plot saccade map.
    myFig = figure('Name', [this_sess.subject ' ' this_sess.ptbfname], 'Position', [200 200 800 600]);
%     plotSaccadeMap(ptb, analysisParams);
    
    behavOutput = procBehavBlocks(ptb.eData.trial);
    plotBehav(behavOutput)
%     
%     save_path = fullfile(paths.results, 'Figures', 'Behaviour', [this_sess.subject '_' this_sess.ptbfname(1:end-7)]);
%     savefig(myFig, save_path);
%     saveas(myFig, save_path, 'tiff');
