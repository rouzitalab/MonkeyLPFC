...
%     c = -(ztpr+zfpr)/2;
    ...
    for fs_ix = 1:nFeatureSets
        ...
        %Only use blocks with many trials
        minTPB = 30;
        nPerBlock = zeros(length(block_uq), 2);
        for block_ix = 1:length(block_uq)
            thisBool = block_id == block_uq(block_ix) & d'>1;
            uq_classes = unique(allTrueY(thisBool));
            for class_ix = 1:length(uq_classes)
                nPerBlock(block_ix, class_ix) = sum(allTrueY(thisBool) == uq_classes(class_ix));
            end
        end
        block_uq = block_uq(~any((nPerBlock<minTPB), 2));
