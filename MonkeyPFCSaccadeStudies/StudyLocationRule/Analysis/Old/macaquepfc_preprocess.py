...
2. Get instantaneous spike rate
...
for sess_name, data_file in file_pairs:
    ...
    data_pkt = nn.Segmentation(data=data_pkt, time_bounds=[-0.2, 1.799])()

    # It's a shame that segmentation makes our sparse matrix full (bool).
    # TODO: Sparse representation of segmented STs

    ana_pkt = nn.ExtractStreams(data=data_pkt, stream_names=['analogsignals'])()
    ana_pkt = nn.Detrend(data=ana_pkt)()

    spk_pkt = nn.ExtractStreams(data=data_pkt, stream_names=['spiketrains'])()
    ...
    data_pkt = nn.MergeStreams(data1=data_pkt, data2=ana_pkt, data3=spk_pkt, replace_if_exists=True)()
    data_pkt = nn.ExtractStreams(data=data_pkt, stream_names=['analogsignals',
                                                              'spiketrains',
                                                              # 'waveforms',  # don't save to h5
                                                              'segmented-markers',
                                                              'spikerates'])()
    nn.ExportH5(data=data_pkt, filename=os.path.join(data_root, sess_name, sess_name + '_segs.h5'))()
