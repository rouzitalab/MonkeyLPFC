...
% critFlip          - The name of the event after which to search for a
%                     saccade. See my_consts.m DEF.flipEvent
...
ana_win(end+1).name = 'bcd';  % Useful for gray non-epoch in raster plot
ana_win(end).timeLockEvent = 'targetOnset';
ana_win(end).winEdges = [-249 1250];
ana_win(end).plotX = [-249 1250];
ana_win(end).avoidEvent = 'cueOffset';
ana_win(end).avoidWin = [-200 Inf];
ana_win(end).vBar = [];
ana_win(end).colour = [0.5 0.5 0.5];
...
analysisParams = struct(...
    ...
    'binWidth', 50,...
    'kernSD', 50,...
    'classifyTarg', 'sacPol',...  % 'class' 'cueColour' 'sacPol' 'sacClass' 'targPol' 'targClass'
    'minTrialsPerGroup', 15,...
    'useUnits', 'sorted',...  % 'all' 'sorted' 'merged'
    'ptest', 0.1,...
    'statPerm', 1001,...  % Number of permutations for statistics
...
[~, lib] = ismember({'baseline' 'cue' 'delay' 'response'}, {ana_win.name});
analysisParams(end).anaWins = ana_win(lib);

analysisParams(end+1) = analysisParams(strcmpi({analysisParams.name}, 'tuning'));
analysisParams(end).name = 'raster';
[~, lib] = ismember({'bcd', 'baseline' 'cue' 'delay' 'response'}, {ana_win.name});
analysisParams(end).anaWins = ana_win(lib);

%timeseries
analysisParams(end+1) = analysisParams(strcmpi({analysisParams.name}, 'tuning'));
analysisParams(end).name = 'timeseries';
analysisParams(end).binWidth = 250;
...
[~, lib] = ismember({'trajectory' 'response'}, {ana_win.name});
analysisParams(end).anaWins = ana_win(lib);

%For trajectory MI
analysisParams(end+1) = analysisParams(strcmpi({analysisParams.name}, 'tuning'));
analysisParams(end).name = 'trajOnly';
% analysisParams(end).useUnits = 'all';
analysisParams(end).binWidth = 50;
...
% analysisParams(end).useUnits = 'all';
[~, lib] = ismember({'baseline' 'cue' 'delay' 'fullDelay' 'response' 'fullTrial' 'trajectory'}, {ana_win.name});
analysisParams(end).anaWins = ana_win(lib);
...
% analysisParams(end).useUnits = 'all';
[~, lib] = ismember({'fullTrial'}, {ana_win.name});
analysisParams(end).anaWins = ana_win(lib);
